...
| **Phase 5** | Validation on real-world Xuancheng AVI dataset (Ma et al. 2026) | `sumo_validation/score_seed_big.py --dataset xuancheng` | Rush/off-peak: fitted detector at chance for K ∈ {8,16,32,64} even when the full-observation reference weakly carries signal (iter 26 retracted the "75% OD-mix" framing). Labour-vs-normal stress test: stronger fine-OD empirical signal **0.567**, but fitted detector **0.521** and multistart-selected **0.502**. Empirical 2-step backoff on the same split gives no lift (best is λ=0). Tier-3 negative result now survives both a stronger proxy and a simple route-memory audit. | iters 20–22, 25–26, 30–31 |
...
Rush-vs-off-peak: fitted detector at chance for K ∈ {8, 16, 32, 64}. The original "~75% OD-mix" framing from iter 21 was **retracted in iter 26** as a K=8 artefact; at finer K the full-observation reference returns to ~0.55–0.58, but fitted remains chance. Labour-vs-normal stress test (iter 30): pooled holiday is weak, but Labour-only at K=64 has a real fine-OD empirical signal (AUC 0.567, CI [0.521, 0.610]); zero-init fitted_f is only 0.521 with Pearson +0.066, and K=5 multistart validation-selection drops fitted_f to 0.502. Iter 31 empirical 2-step backoff on that same Labour K=64 split gives no route-memory lift: all non-zero 2-step weights reduce AUC, best is λ=0 (the original 1-step chain). Robust finding: stronger real-world behavioural signal exists, but the 1-step partial-observation detector does not recover it robustly, and simple 2-step memory is not the immediate Xuancheng rescue.
...
3. **Holiday/Labour split was a useful negative stress test, not a rescue.** Pooled holiday (Apr 5 + Apr 28–30) has empirical AUC near chance. Labour-only vs normal has stronger empirical signal after fine OD matching (K=64 AUC 0.567), but fitted_f is only 0.521 and validation-selected multistart drops it to 0.502. A full-observation 2-step backoff audit does not improve on 0.567. So the Xuancheng limitation is weak partial-observation recovery / proxy structure, not merely choosing too weak a proxy or missing simple 2-step route memory.
...
1. **Thesis revision.** First draft complete (2026-05-28). Major late revisions are now wired: §6.5 reflects the iter-28 SUMO 2-step backoff overturn, §6.8 reflects the iter-29 f+g diagnosis, §7 reflects the iter-30/31 Xuancheng Labour and 2-step audits, and §8/§10 carry the updated interpretation. Remaining work is mainly polish/page-budget and any supervisor-format tweaks. See `thesis/README.md` and memory `thesis_writeup_state.md`.
...
5. **Parametric 2-step Morimura inverter.** Iter 28 overturned the original SUMO negative, so this is now a Tier-2 future-work candidate rather than deprioritised. The 2-step empirical with backoff lifts SUMO AUC by +0.10; a parametric extension would let the same lever work in the partial-observation regime. Iter 31 checked Xuancheng Labour K=64 and found no empirical 2-step lift, so this should be framed as simulation-motivated model-class work, not as the confirmed Xuancheng remedy.
