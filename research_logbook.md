...
- **Parametric family at scale (iter-11 finding).** Local-only softmax-parametric family is insufficient to fit SUMO's transition kernel at $n = 2{,}405$ (39 % gap closure vs 97 % at $n = 686$). Three options to push the bigger-bbox `fitted_f` higher: (a) add global-feature parameters (paper §3.2; currently omitted per `morimura.py` docstring); (b) replace the parametric family with a non-parametric or graph-neural estimator at scale; (c) abandon the bigger-bbox direction and use the small-bbox row as the headline validation.
