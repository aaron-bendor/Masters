...
The iter-8 headline `fitted_f` = 0.714 **does not stand as a clean Morimura validation**. It was the framework correctly extracting signal from a mix of (driving patterns + teleport artifacts + finisher-selection bias). After iter 9–13 we have two complementary statements at clean demand:

| Setup | n | T | empirical | `fitted_f` local | `fitted_f` w/ globals | Read |
|---|---|---|---|---|---|---|
| Small bbox `-p 2.0`  | 686  | 20 | 0.612 | **0.599** | — | 97 % gap closure — clean validation |
| Bigger bbox `-p 0.5` | 2,405 | 20 | 0.66 | 0.58 ± 0.02 | 0.58 | Globals add ~0.01; `X_o` variance ±0.02 |
| Bigger bbox `-p 0.5` | 2,405 | **40** | **0.809** | **0.615** | (untested) | Empirical reveals much more signal; fitter stuck near 0.6 |

The small-bbox row is the clean **Morimura validation**: the inverter recovers 97 % of available 1-step Markov signal at clean demand, slightly *better* than iter 8's contaminated 96 % — so the framework works as advertised on networks where the parametric family is rich enough.

The bigger-bbox rows together tell a **scaling-limit** story. With 3.5× the edges and a much cleaner simulation (0.1 % satnav teleport, 36 % more satnav trips), the data carries substantially more regime signal — empirical climbs to **0.81 at T = 40** (iter 13). But `fitted_f` is stuck near 0.6 regardless of trajectory length, addition of `phi_T` / `psi` global features (iter 12), or optimiser budget (iter 11). The misspecification ceiling sits around AUC 0.60–0.62 for the local + ω-global softmax family on this network. Closing the remaining ~0.20 AUC of available signal would require a fundamentally richer model class — 2-step Markov, OD-conditional, or non-parametric / graph-neural alternatives.
