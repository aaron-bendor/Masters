...
> **Iter 11 — bigger-bbox `-p 0.5` finds the cleanest regime contrast yet, and exposes a misspecification ceiling.** Demand sweep on the bigger bbox at `-p` ∈ {0.25, 0.5, 1.0}: `-p 0.5` is the clean operating point — 16.5 % intr teleport / **0.1 %** satnav (just 15 teleports against 28,799 inserted vehicles), and satnav delivers **36 % more trips** (28,098 vs 20,700). The cleanest regime contrast we've measured. Detector at maxiter = 300: empirical AUC 0.666 (up from small-bbox 0.612), `fitted_f` 0.565 — only 39 % of the available gap closed (vs 97 % at small bbox). To rule out an L-BFGS-budget issue, surfaced `fit_chain`'s `OptimizeResult` and re-ran at maxiter = 2000 (6.7× more iterations). Loss decreased by only 1.4 % (intr, $101.7 \to 100.3$) and 1.7 % (satnav, $126.8 \to 124.6$); AUC unchanged at 0.565 to three decimals. **The 0.10 gap is not the iteration budget — it's the local-only softmax-parametric family being insufficient to represent SUMO's transition kernel at $n = 2{,}405$.** Two takeaways: (a) the small-bbox 97 % gap closure was real because the parametric family is rich enough at that scale; (b) bigger network gives more recoverable signal (empirical $0.612 \to 0.666$) but the inverter can't access it without enriching the parametric family. More compute does not help.
...
The iter-8 headline `fitted_f` = 0.714 **does not stand as a clean Morimura validation**. It was the framework correctly extracting signal from a mix of (driving patterns + teleport artifacts + finisher-selection bias). After iter 9–11 we have two complementary statements at clean demand:

| Setup | n | empirical | `fitted_f` | gap closed | Read |
|---|---|---|---|---|---|
| Small bbox `-p 2.0`  | 686  | 0.612 | **0.599** | **97 %** | Parametric family adequate; framework excels |
| Bigger bbox `-p 0.5` | 2,405 | 0.666 | 0.565 | 39 %  | Misspecification at scale; local softmax insufficient |

The small-bbox row is the clean **Morimura validation**: the inverter recovers 97 % of available 1-step Markov signal — slightly *better* than iter 8's contaminated 96 % — so the framework itself is fine. What dropped is the *ceiling*: from teleport-inflated 0.747 to honest 0.612. Available regime signal at small scale is real but modest (0.10 above chance).

The bigger-bbox row is a complementary **scaling-limit** statement. On a network with 3.5× the edges and a much cleaner sim (0.1 % satnav teleport, 36 % more satnav trips), the empirical ceiling climbs by 0.054 — so there IS more recoverable signal at scale. But `fitted_f` stalls at 0.565 and the iter-11 convergence diagnostic confirms this is a model-family limit, not a fitter limit. The local-only softmax-parametric family that Morimura uses can't represent SUMO's transition kernel well enough at the larger scale. Closing this gap would require enriching the parametric family (the omitted global-feature term, or non-parametric / graph-neural alternatives).
...
- **Parametric family at scale (iter-11 finding).** Local-only softmax-parametric family is insufficient to fit SUMO's transition kernel at $n = 2{,}405$ (39 % gap closure vs 97 % at $n = 686$). Three options to push the bigger-bbox `fitted_f` higher: (a) add global-feature parameters (paper Eqs. 17–18; currently omitted per `morimura.py` docstring); (b) replace the parametric family with a non-parametric or graph-neural estimator at scale; (c) abandon the bigger-bbox direction and use the small-bbox row as the headline validation.

  **Decision history for (a) — why local-only was originally chosen, and why now to revisit it.** The local-only choice was made in Phase 1 with the justification "keeps the parameter space transparent for the synthetic test" (deviations table near the top of this logbook). The paper allows the omission explicitly (page 7: *"If a simpler model is preferred, either of them would be omitted."*). The choice was then carried forward unchanged through Phases 2–4 without revisiting. Two things now argue for adding the ω-global term:
    - **Iter-11 misspecification is the failure mode shared-parameter features are designed to mitigate.** Local-only at $n = 2{,}405$ gives 8,392 free parameters against ~28 k trajectories; the data can't pin them down precisely. Tying many edges together via shared road-class / geometry features is the natural inductive bias.
    - **The paper's own real-world experiment (§6.2, Nairobi) uses the ω-global term** — $\phi_T(x)$ from road category, $\psi(x, x')$ from turn angle — while still omitting the ν-global term. That is the precedent we would be matching: keep ν local-only for the initial distribution, add globals to the transition kernel. SUMO's `net.net.xml` provides road class, speed limit, lane count, length, and geometry for free, so feature engineering is trivial.

  **Foundations verified before extending.** Ran finite-difference checks on `Inverter.grad_log_pi`, `Inverter.grad_log_h`, and the full `loss_grad` across three random seeds: max relative error ≈ $5 \times 10^{-9}$ on all three (at the FD truncation floor for `eps = 1e-6`). `stationary` and `hitting_pack` recursions hold to machine precision (~$10^{-16}$). The local-only solver is a numerically-exact implementation of paper Eqs. 2–15; extending to ω-globals can build on this base with confidence.
