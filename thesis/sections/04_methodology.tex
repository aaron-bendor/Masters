...
the $1$-step model class can close the gap to the data ceiling.
...
\[
\Delta_{\mathrm{fit}}(x, \cdot) = \pT^{(B)}_{\mathrm{fit}}(x, \cdot) - \pT^{(A)}_{\mathrm{fit}}(x, \cdot),
\qquad
\Delta_{\mathrm{emp}}(x, \cdot) = \pT^{(B)}_{\mathrm{emp}}(x, \cdot) - \pT^{(A)}_{\mathrm{emp}}(x, \cdot),
\]
where the empirical chain $\Pchain_{\mathrm{emp}}$ is the
