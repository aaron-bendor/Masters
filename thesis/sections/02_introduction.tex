...
a per-trajectory log-likelihood-ratio detector. Augmenting the inverter
with real SUMO edge features robustly improves \emph{per-row chain
recovery} across a five-seed sweep (stacked Pearson $+0.050 \pm 0.023$,
$p \approx 0.008$), although the aggregate detection-AUC lift is
seed-dependent rather than robust. On the representative seed the
area under the ROC curve at $T=20$ test transitions reaches
$\AUC = 0.644$, against an empirically verified $1$-step Markov data
ceiling of $0.685$ (closing $77.5\%$ of the recoverable gap); a
trajectory-length sweep lifts the ceiling and the detector together to
$\AUC = 0.708$ at $T=40$ against a ceiling of $0.801$.
