...
driver.
\item[\code{congestion\_filter.py}] Tier-2-precursor experiments
(Phase~2 of the project's internal phasing): snapshot-level
...
arguments before they made it into the writeup. The same assistant
contributed to drafts of this report, working from the author's
running \code{research\_logbook.md} and the experimental result logs;
the author edited every section for accuracy and final framing. All
experimental decisions, all interpretations of results, and the
choice of what counts as an honest finding remained with the author.
Specifically the decision to refuse the holiday-vs-normal regime
proxy in favour of the OD-matched rush-vs-off-peak contrast
(\cref{sec:phase5-proxy,sec:reflect-skills}) was made against the
assistant's initial suggestion, on methodological grounds.
